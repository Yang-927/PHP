<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:78:"C:\phpstudy_pro\WWW\1809test\public/../application/index\view\index\goods.html";i:1575425159;}*/ ?>
	...
	<div class="shop_goods_bd clear">
		...
		<div class="shop_goods_show">
			<div class="shop_goods_show_left">
				...
				<div id="preview">
					<div class=jqzoom id="spec-n1" onClick="window.open('/')"><IMG height="350" src="<?php echo $goods_list['goods_img']; ?>" jqimg="<?php echo $goods_list['goods_img']; ?>" width="350">
						</div>
						<div id="spec-n5">
							...
							<div id="spec-list">
								<ul class="list-h">
									<li><img src="<?php echo $goods_list['goods_img']; ?>"> </li>	
								</ul>
							</div>
			...
			<div class="shop_goods_show_right">
				<ul>
					<li>
						<strong style="font-si ze:14px; font-weight:bold;"><?php echo $goods_list['goods_name']; ?></strong>
					</li>
					<li>
						<label>价格：</label>
						<span><strong><?php echo $goods_list['price']; ?></strong>元</span>
					</li>
					<li>
						<label>运费：</label>
						<span>卖家承担运费</span>
					...
					<li style="padding:20px 0;">
						<label>&nbsp;</label>
						<span><a href="javascript:void(0);" class="goods_sub goods_sub_gou" >加入购物车</a></span>
						
							<span><a href="javascript:void(0);" class="collect" is_red='<?php echo $goods_list['is_red']; ?>'>收藏</a></span>
...
<script type="text/javascript">
	...
	$('.goods_sub_gou').click(function(){
		var id = <?php echo $goods_list['id']; ?>;
		var number = $('#good_nums').val();
		$.ajax({
			url:'/index/Cart/cart_add',//接口URL
			type:'post',//传值方式
	...
	//请在此处编辑代码
	// 收藏
	var is_red = $('.collect').attr('is_red');
	console.log(is_red)
	if(is_red == 1){
		$('.collect').attr('id','collect_no').text('取消收藏');
	}else{
		$('.collect').attr('id','collect')
	}
	$('#collect').click(function(){
		var goods_id = <?php echo $goods_list['id']; ?>;
		$.ajax({
			url:'/index/Index/add_favo',
			type:'post',
			dataType:'JSON',
			...
			success:(res) => {
				if(res.error_code == 0){
					...
					alert(res.error_msg);
					$(this).attr("id","collect_no").text('取消收藏')
					location.href='/index/Index/goods_detail?id=<?php echo $goods_list['id']; ?>';
				}
			}
		})
	})
	$('#collect_no').click(function(){
		var goods_id = <?php echo $goods_list['id']; ?>;
		$.ajax({
			url:'/index/Index/del_favo',
			type:'post',
			dataType:'JSON',
			data:{"goods_id":goods_id},
			success:(res) => {
